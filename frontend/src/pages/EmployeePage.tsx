...
import { ArrowRight, CalendarDays, Check, CheckCircle2, Clock3, Compass, Lightbulb, RotateCcw, Sparkles, Target, XCircle } from 'lucide-react'
...
const traceSteps = ['Анализируем профиль и цель', 'Сравниваем навыки с требованиями', 'Учитываем историю участия', 'Составляем план развития']
...
export default function EmployeePage({ context }: { context: ApiContext }) {
  ...
  useEffect(() => { if (!recommending) return; const timer = window.setInterval(() => setTraceIndex(i => Math.min(i + 1, traceSteps.length - 1)), 850); return () => clearInterval(timer) }, [recommending])
  useEffect(() => { if (!toast) return; const timer = window.setTimeout(() => setToast(''), 4500); return () => clearTimeout(timer) }, [toast])

  const recommend = async () => {
    setRecommending(true); setTraceIndex(0); setError('')
  ...
  const decline = async (eventId: string) => {
    setBusy(eventId); setError('')
    try { await api.feedback(context.employeeId, eventId, context); setToast('Поняли. Подберём другой шаг.'); setResult(await api.recommend(context.employeeId, context)) }
    catch (cause) { setError((cause as Error).message) }
    finally { setBusy(null) }
  }

  const skills = useMemo(() => profile ? skillsOf(profile).sort((a, b) => Number(b.critical) - Number(a.critical) || (b.required_level - b.effective_level) - (a.required_level - a.effective_level)) : [], [profile])
  ...
  return <>
    ...
    <div className="employee-layout"><div className="main-column">
      ...
      <section className="card"><div className="section-title"><div><span className="icon-chip"><Clock3 size={19} /></span><h2>История участия</h2></div><span className="muted small">{history.length} записей</span></div>{history.length ? <div className="history-list">{history.slice().reverse().slice(0, 8).map((item, index) => <div key={`${item.event_id}-${index}`} className="history-item"><span className={`history-dot ${item.status}`} /><div><strong>{item.title ?? item.event_id}</strong><span>{item.date ?? 'Дата не указана'}</span></div><span className={`status ${item.status}`}>{statuses[item.status] ?? item.status}</span></div>)}</div> : <p className="muted">История участия появится после первой активности.</p>}</section>
      ...
      <section className="card recommendation-panel"><div className="recommendation-head"><div className="sparkle-icon"><Sparkles size={23} /></div><span className="eyebrow">AI-НАВИГАТОР</span><h2>Куда двигаться дальше?</h2><p>Учитываем цель, разрывы навыков, пройденные шаги и формат работы.</p><button className="button primary wide" onClick={recommend} disabled={recommending || !!busy}>{recommending ? 'Подбираем шаги…' : result ? 'Обновить рекомендации' : 'Подобрать шаги'}<ArrowRight size={17} /></button></div>
        {recommending && <div className="trace-panel"><span className="eyebrow">ХОД АНАЛИЗА</span>{traceSteps.map((step, index) => <div className={`trace-step ${index <= traceIndex ? 'shown' : ''}`} key={step}><span>{index < traceIndex ? <Check size={13} /> : index === traceIndex ? <div className="mini-spinner" /> : index + 1}</span>{step}</div>)}</div>}
        {result && !recommending && <div className="recommendation-results">{result.trace?.length ? <details className="trace-details"><summary>Как агент пришёл к решению</summary>{result.trace.map((step, index) => <p key={index}>{typeof step === 'string' ? step : step.message ?? step.step ?? step.tool ?? step.detail}</p>)}</details> : null}{recs.length ? recs.map((rec, index) => <RecommendationCard key={`${rec.event_id}-${index}`} rec={rec} index={index} busy={busy} onComplete={complete} onDecline={decline} />) : <div className="empty-mini"><CheckCircle2 size={24} /><strong>Подходящих шагов сейчас нет</strong><p>Проверьте цель или доступные мероприятия позже.</p></div>}{result.why_not || result.rejected ? <div className="why-not"><Lightbulb size={18} /><div><strong>Почему не другой шаг?</strong><p>{result.why_not ?? (typeof result.rejected === 'string' ? result.rejected : result.rejected?.map(item => `${item.title ?? item.event_id}: ${item.reason ?? item.rationale ?? 'сейчас менее полезно'}`).join('; '))}</p></div></div> : null}</div>}
      </section>
